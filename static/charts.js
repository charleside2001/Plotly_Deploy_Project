...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var resultSample = charts.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    var resultChart = resultSample[0];
    console.log(resultChart);
    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otuIds = resultChart.otu_ids;
    console.log(otuIds);
    var otuIdsSlice = otuIds.slice(0, 10).reverse();
    console.log(otuIdsSlice);
    var sampleValues = resultChart.sample_values;
    console.log(sampleValues);
    var sampleValuesSlice = sampleValues.slice(0, 10).reverse();
    console.log(sampleValuesSlice);
    var otuLabels = resultChart.otu_labels;
    ...
    otuIdsSlice.forEach(function(ids){
      console.log(ids);
      otuLabel.push("OTU " + ids);
